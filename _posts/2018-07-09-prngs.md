...
From now on we'll use a PRNG of a higher quality, the Mersenne Twister. The more recent [PCG PRNG](http://www.pcg-random.org/) should be better f and it's what I use in my rendering system. Having said that, R's runif (short for random uniform) command uses the Mersenne Twister algorithm, and for these posts it should suffice. PBRT 2 used to use Mersenne Twister and PBRT V3 uses PCG. 
